...
export function handleSearch(indexManager: IndexManager) {
  return async ({ university, query }: { university: string; query: string }) => {
    ...
    const lines = results.map(
      (r) => `- [id=${r.id}] ${r.name} (버전 ${r.allVersions.length}개)`,
    );

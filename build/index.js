...
server.registerTool("search_regulations", {
    description: "대학 규정을 키워드로 검색합니다. 규정명에서 키워드를 찾아 매칭되는 규정 목록을 반환합니다.",
    inputSchema: searchInputSchema,
}, handleSearch(indexManager));
server.registerTool("list_regulations", {
    description: "대학 규정 목록을 반환합니다. university를 생략하면 지원 대학 목록을 보여줍니다.",
    inputSchema: listInputSchema,
}, handleList(indexManager));
server.registerTool("get_regulation", {
    description: "특정 규정의 전문을 조회합니다. search_regulations 또는 list_regulations에서 얻은 id(SEQ_HISTORY)를 사용하세요.",
    inputSchema: getContentInputSchema,
}, handleGetContent());
server.registerTool("get_regulation_article", {
    description: "특정 규정의 특정 조항만 조회합니다. 규정 id와 조항 번호(예: '제5조')를 입력하세요.",

...
const cache = new Map();
function cacheKey(universityId, seqHistory) {
    return `${universityId}:${seqHistory}`;
}
/**
 * Fetch regulation HTML by university and SEQ_HISTORY id.
 */
export async function fetchRegulationHtml(universityId, seqHistory) {
    ...
    if (!html.includes('class="lawname"'))
        return null;
    cache.set(key, html);
    return html;
}
 ...
 */
export async function probeRegulationName(universityId, seqHistory) {
